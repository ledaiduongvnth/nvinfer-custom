...
NvDsMsgApiErrorType nvds_kafka_read_config(NvDsKafkaClientHandle *kh, char *config_path, char *partition_key_field, int field_len, rd_kafka_conf_t *conf)
{
  ...
  gchar **key = NULL;
  GError *error = NULL;
  char *confptr = NULL, *p_confptr = NULL , *c_confptr = NULL;
  ...
  for (key = keys; *key; key++) {
    ...
    if (!g_strcmp0(*key, CONFIG_GROUP_MSG_BROKER_RDKAFKA_CFG))
       {
          ...
          if (error) {
             free_gobjs(key_file, error, keys, setvalquote);
    ...
    // check if this is one of adaptor settings
    if (!g_strcmp0(*key, CONFIG_GROUP_MSG_BROKER_RDKAFKA_PRODUCER_CFG))
       {
           setvalquote = g_key_file_get_string (key_file, CONFIG_GROUP_MSG_BROKER,
               CONFIG_GROUP_MSG_BROKER_RDKAFKA_PRODUCER_CFG, &error);
          ...
          p_confptr = setvalquote;
          size_t conflen = strlen(p_confptr);

          //remove "". (string length needs to be at least 2)
          //Could use g_shell_unquote but it might have other side effects
          if ((conflen <3) || (p_confptr[0] != '"') || (p_confptr[conflen-1] != '"')) {
             nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR,  "invalid format for rdkafa \
                               producer config entry. Start and end with \"\"\n");
            free_gobjs(key_file, error, keys, setvalquote);
            return NVDS_MSGAPI_ERR;
           }
           p_confptr[conflen-1] = '\0'; //remove ending quote
           p_confptr = p_confptr + 1; //remove starting quote
          nvds_log(NVDS_KAFKA_LOG_CAT, LOG_INFO,  "kafka producer setting %s = %s\n", *key, p_confptr);
       }
    // check if this is one of adaptor settings
    if (!g_strcmp0(*key, CONFIG_GROUP_MSG_BROKER_RDKAFKA_CONSUMER_CFG))
       {
           setvalquote = g_key_file_get_string (key_file, CONFIG_GROUP_MSG_BROKER,
               CONFIG_GROUP_MSG_BROKER_RDKAFKA_CONSUMER_CFG, &error);
          ...
          c_confptr = setvalquote;
          size_t conflen = strlen(c_confptr);

          //remove "". (string length needs to be at least 2)
          //Could use g_shell_unquote but it might have other side effects
          if ((conflen <3) || (c_confptr[0] != '"') || (c_confptr[conflen-1] != '"')) {
             nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR,  "invalid format for rdkafa \
                             consumer config entry. Start and end with \"\"\n");
            free_gobjs(key_file, error, keys, setvalquote);
            return NVDS_MSGAPI_ERR;
           }
           c_confptr[conflen-1] = '\0'; //remove ending quote
           c_confptr = c_confptr + 1; //remove starting quote
          nvds_log(NVDS_KAFKA_LOG_CAT, LOG_INFO,  "kafka consumer setting %s = %s\n", *key, c_confptr);
       }

       // check if this entry specifies the partition key field
    if (!g_strcmp0(*key, CONFIG_GROUP_MSG_BROKER_PARTITION_KEY))
        {
           gchar *key_name_conf = NULL;
           key_name_conf = g_key_file_get_string (key_file, CONFIG_GROUP_MSG_BROKER,
                CONFIG_GROUP_MSG_BROKER_PARTITION_KEY, &error);
           if (error) {
             nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR,  "Error parsing config file\n");
             free_gobjs(key_file, error, keys, key_name_conf);
             return NVDS_MSGAPI_ERR;
           }
          if(key_name_conf==NULL || !strcmp(key_name_conf,"")) {
    ...
    //Fetch consumer group name
    if (!g_strcmp0(*key, CONFIG_GROUP_MSG_BROKER_CONSUMER_GROUP)) {
        gchar *key_name_conf = g_key_file_get_string (key_file, CONFIG_GROUP_MSG_BROKER, CONFIG_GROUP_MSG_BROKER_CONSUMER_GROUP, &error);
        if(error) {
            nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR,  "Error parsing config file\n");
            free_gobjs(key_file, error, keys, key_name_conf);
            return NVDS_MSGAPI_ERR;
        ...
        if(key_name_conf==NULL || !strcmp(key_name_conf,"")) {
            nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR,  "Consumer group id not specified in cfg. Using default group id: %s \n", DEFAULT_KAFKA_CONSUMER_GROUP);
        }
        else {
            strcpy(kh->c_instance.consumer_grp_id, key_name_conf);
        }
        g_free(key_name_conf);
    }
  }
  free_gobjs(key_file, NULL, keys, NULL);
  ...
  if (confptr) {
      ...
      if(nvds_kafka_parse_proto_cfg(confptr, conf) == NVDS_MSGAPI_OK) {
          kh->c_instance.config = string(confptr);
      }
      else {
  ...
  if (p_confptr) {
      if(nvds_kafka_parse_proto_cfg(p_confptr, conf) != NVDS_MSGAPI_OK) {
  ...
  if (c_confptr) {
      if(kh->c_instance.config != "")
          kh->c_instance.config = kh->c_instance.config + ";" + string(c_confptr);
      else
          kh->c_instance.config = string(c_confptr);
  }
  return NVDS_MSGAPI_OK;
}
...
NvDsMsgApiHandle nvds_msgapi_connect(char *connection_str,  nvds_msgapi_connect_cb_t connect_cb, char *config_path)
{
  ...
  nvds_log(NVDS_KAFKA_LOG_CAT, LOG_INFO, "kafka broker url = %s; port = %s;", burl.c_str(), bport.c_str());
...
NvDsMsgApiErrorType nvds_msgapi_subscribe (NvDsMsgApiHandle h_ptr, char ** topics, int num_topics, nvds_msgapi_subscribe_request_cb_t cb, void *user_ctx) {
    ...
    if(kh->c_instance.consumer) {
        nvds_log(NVDS_KAFKA_LOG_CAT, LOG_ERR, "Kafka Subscription already enabled. nvds_msgapi_subscribe() can be called only once\n");
        return NVDS_MSGAPI_ERR;
    }
